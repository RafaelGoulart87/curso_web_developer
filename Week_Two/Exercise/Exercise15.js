...
Array de Objetos;
Exemplo de Entrada:
[{nome: 'Marcos', notas: [10, 7, 2]},{nome: 'Michele', notas: [10, 10, 10]},{nome: 'Pedro', notas: [3, 10, 9]},{nome: 'Andressa', notas: [6, 6, 8]},
...
const student = (user) => {
        ...
        for (let i = 0; i < user.length; i++) {
            ...
            for (let j = 0; j < user[i].notas.length; j++) {
                let nota = user[i].notas[j]
                sum += nota
            }
            let media = parseInt(sum / user[i].notas.length)
            if (media >= 7) {
                console.log(user[i].nome + " aprovado(a) com média " + media)
            }
        }
    }
    (student([
